...
def _get_capabilities(prepared_dataset: dict[str, Any]) -> dict[str, bool]:
    """Backward-compatible alias for deterministic capability detection."""
    return detect_analysis_capabilities(prepared_dataset)
...
def _build_orchestration_context(
    prepared_dataset: dict[str, Any],
    capabilities: dict[str, bool],
    supported_agents: set[AgentName] | None = None,
) -> dict[str, Any]:
    """Backward-compatible wrapper for the public compact-context builder."""
    del supported_agents
    return build_orchestration_context(prepared_dataset, capabilities)
...
def _fallback_plan(supported_agents: set[AgentName]) -> OrchestrationPlan:
    """Backward-compatible alias for the deterministic routing plan."""
    return _deterministic_routing_plan(supported_agents)
