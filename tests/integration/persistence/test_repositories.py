...
class FallbackVectorStore(SupabaseVectorStore):
    def __init__(self) -> None:
        super().__init__(SupabaseGateway(settings=Settings("", "")))
...
def test_create_dataset_falls_back_when_metadata_columns_are_not_migrated() -> None:
    gateway = SupabaseGateway(settings=Settings("", ""))
    service = AnalysisRepository(gateway)
...
def test_replace_document_chunks_uses_one_atomic_rpc() -> None:
    gateway = SupabaseGateway(settings=Settings("", ""))
    service = SupabaseVectorStore(gateway)
...
def test_replace_document_chunks_keeps_legacy_projects_working() -> None:
    service = FallbackVectorStore()
    service.gateway._client = MissingRpcClient()
